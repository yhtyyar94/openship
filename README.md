...
Compose starts a PostgreSQL database and the app at [http://localhost:3000](http://localhost:3000). To use a different local port, run `APP_PORT=3001 docker compose up --build`.

On startup, the app container runs `prisma migrate deploy` before starting Next.js.
